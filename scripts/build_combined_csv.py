...
TTLOCK = "automation-data/ttlock_log.csv"
...
def main():
    ...
    ttlog = pd.read_csv(TTLOCK) if os.path.exists(TTLOCK) else pd.DataFrame(columns=["reservation_code"])
    ...
    # Identify Booking.com or Expedia reservations → deposit required
    ...
    # -----------------------------
    # RULE C: IGNORE far-future & past bookings
    # -----------------------------
    valid = bookings[
        (bookings["check_out"] >= today) &
        (bookings["check_in"] <= future_cutoff)
    ].copy()

    print(f"➡ {len(valid)} bookings fall within the valid window.")
