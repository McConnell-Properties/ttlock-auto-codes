...
def main():
    ...
    # ==========================================
    # PHASE 1A: LOAD EXISTING LOGS (ONE ROW PER BOOKING)
    # ==========================================
    if os.path.exists(LOG_PATH):
        try:
            df_old = pd.read_csv(LOG_PATH, dtype=str)
            print(f"📖 Found existing log file with {len(df_old)} entries.")
                ...
                if status in ["link_generated", "hold_active"] and session_id:
                    try:
                        ...
                        if new_status != status:
                            row_dict["stripe_status"] = new_status
    ...
    for ref, state in bookings_state.items():
        ...
        # Keep historical bookings unmodified (1 line)
        ...
        current_start_ms = str(state.get("start_ms", "")).strip()
        current_end_ms = str(state.get("end_ms", "")).strip()
        ...
        # Sync final timestamps so they trigger bypass next time
