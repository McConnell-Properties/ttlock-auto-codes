...
def main():
        ...
        try:
            ...
            for index, row in df_old.iterrows():
                ...
                if not ltype:
                    ...
                    cc_val = str(row_dict.get("code_created", "no")).strip().lower()
                    if cc_val == "nan" or not cc_val: cc_val = "no"

                    bookings_state[ref]["locks"][ltype] = {
                        "code_created": cc_val,
                        "ttlock_response": str(row_dict.get("ttlock_response", "")).strip(),
                        "start_ms": str(start_ms_val).strip(),
                        "end_ms": str(end_ms_val).strip(),
                        "ttlock_start": t_start_val,
                        "ttlock_end": t_end_val
                    }
                # ----------------------------------------------
    ...
    for ref, state in bookings_state.items():
        ...
        for ltype in ["front_door", "room"]:
            ...
            # SAFEGUARD: Eliminate formatting bugs by standardizing inputs to boolean and integers
            is_created = l_data["code_created"] in ["yes", "true"]
            try: csv_start = int(float(l_data["start_ms"]))
            except: csv_start = 0
            try: csv_end = int(float(l_data["end_ms"]))
            except: csv_end = 0

            if is_created:
                # Compare pure integers
                if csv_start == start_ms and csv_end == end_ms:
                    ...
                    if pwd_id:
                        ...
                        is_created = False # Force recreation
